...
void init(int n){	// init the components as distinct initially
    rep(i ,1 , n+1){
        p[i] = i;
        rnk[i]=1;
    }
...
void merge(int x, int y){	// rank compression
    x=find(x);
    y=find(y);
...
int32_t main(){
    __;	
    memset(p , -1 , sizeof(p)); //init the roots 
    int n; cin >> n;
    init(n);                    // init all distinct components
    return 0;
}
